...
namespace Recurrence {
    ...
    public:
        ...
        /*
         * Computes x_t mod mod for the recurrence:
        ...
        T solve(long long int t, T mod) const {
            using NumberTheory::normalize_mod;

            const int n = static_cast<int>(initialValues.size());

            if (t < n) {
                return normalize_mod<T>(initialValues[t], mod);
            }
             ...
             */
            for (int i = 1; i <= n; i++) {
                transition[0][i - 1] = normalize_mod<T>(coefficients[i], mod);
            }
            transition[0][n] = normalize_mod<T>(coefficients[0], mod);
            ...
            for (int i = 0; i < n; i++) {
                state[i][0] = normalize_mod<T>(initialValues[n - i - 1], mod);
            }
            state[n][0] = normalize_mod<T>(1, mod);
